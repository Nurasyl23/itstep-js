...
/*const selectElement = document.querySelector('.home__list');
...
const createGame = (gameDiv, timerDiv, gameOverCallback) => {
  ...
  const state =  generateState(N, N);
  const timers = generateTimersList(N, N);
  ...
  const gridDiv = $("div", { 
    ...
    ...createArray(index => createCard(index, String(values[index])))(N * N)
  );
